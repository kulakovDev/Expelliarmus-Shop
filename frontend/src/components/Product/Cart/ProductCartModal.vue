            ...
            >
              ...
              <div v-if="products.length > 0">
                <div class="overflow-y-auto max-h-72">
                  <div class="mt-10 space-y-8" v-for="product in products" :key="product.id">
                    <div class="flex justify-between items-center gap-x-8 relative px-6">
                      <div class="pl-4">
                        <product-checkout-card :price="'$'+product.subTotal" :product-name="product.name" :image="product.image"></product-checkout-card>
                      </div>
                      <quantity-adjuster v-model="product.quantity"></quantity-adjuster>
                      <button class="absolute top-3.5 left-0 text-red-600" @click="removeProduct(product.id)">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
...
export default defineComponent({
  components: {QuantityAdjuster, ProductCheckoutCard, DialogTitle, DialogPanel, TransitionRoot, TransitionChild, Dialog },
  props: {
    isOpen: {
      type: Boolean,
      default: false
